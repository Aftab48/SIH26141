...
- ABORTS: `Charlie no verdict` is a REFUSAL, not a rejection, and it is never folded into one. Under the before-forwarding ordering the recipient-forgery rows are ALL refusals: a substituted declaration leaves the pooled matched count undefined, both verifiers abort, and the attack is a denial of transfer rather than a detected forgery.
