...
turned up an eighth route, where a malformed payload raises on a key round and is discarded on
a check round, so "did the run survive" is the branch; that one was reported rather than fixed
at the time and has since been **closed in Phase 4**, along with a ninth found by the test
written to close it. Both, and the invariant behind the whole family, are in §12. Nothing an honest or an attacked run computes
...
| --- | --- | --- | --- |
| `isolation.py` | n/a (the check) | n/a | `check_attack_isolation`, `assert_attack_isolated`, `IsolationReport`, `AttackIsolationError`, `AttackBuilder`, `DecisionProbe`, `SignerScenario`, `signer_scenario`, `signer_probe`, `forwarder_probe`, `canonical`, `SessionEnvironment`, `active_session`, `session_environment`, `same_stream`, `stream_fingerprint`, `derived_from_seed`, `require_distinct_streams`, `MIN_JUSTIFICATION`, `DEFAULT_*_SEEDS`, `SCENARIO_*`, `SESSION_MATERIAL_BYTES` |
| `statistics.py` | n/a (the arithmetic) | n/a | `wilson_bounds`, `agrees_within`, `AgreementVerdict`, `sigma_tolerance`, `binomial_standard_error`, `two_sided_z`, `Z_90`/`Z_95`/`Z_99` |
...
same verdicts, same pooled count, same spent rounds), and that identity is asserted, because
an ordering that changed an honest run would be changing the protocol rather than naming an
experiment.
...
`1.3` sd low while quoting it to four figures. The four-row table above replaces it, and
those rows carry their own counts and re-check their own arithmetic at import.
...
which for the shipped choice lie in the x-z plane). `params.sifted()` reduces the signing
length accordingly, so the floors and bounds of Phase 2 apply to what is left rather than to
the nominal `L`.
...
Four flavours. Three are closed and the fourth isn't closed by what everyone assumed.
...
| (c) burn a verifier's round | n/a | see below | **not the ledger** |
...
still reaches his own verdict (his own count comes from his own record, and only the
...
offered moved with the session seed all by itself; check (a), which varies exactly that
...
its decisions do not move with the check plan*. The second half is a property of the seam
...
* **A `payload_map` probe used to need `check_fraction = 0`, and the reason was a defect.**
  ...
  consulted* moved with the seed even for a perfect adversary; the same fact let any
...
4. **CHSH cannot attribute at demo check sizing.** `dS = 2√2·p` is only `0.3960` at
   ...
   CHSH needs the full-scale sample and must say so. The per-link deal (§4) halved the
...
logs, where each link now carries half the reserved positions; every observation that
...
be flaky by construction. §9.4's two CHSH half-widths are **derived** (the algebra is printed
with them), and the 300-seed distribution quoted against them is a measurement made once, for
the correction, and not re-run by the suite.
...
| G | **timing** | a check round measures both wings; a key round teleports and measures one. The wall-clock gap between consecutive seam calls classifies the branch, with no instrumentation of any kind | **open, and out of scope for this section** |
...
check rounds, `0.3659 s` before and `0.3703 s` after without. Those differences are smaller
than the run-to-run spread, in both directions. Both directions of the *leak* are measured too:
...
Paying that would buy less than it looks like. Equal *work* isn't constant *time*. Two Python
...
**survived** (H). Each time, the next spelling appeared, which is the signature of patching
...
> **interaction trace**, which is the calls it receives, the reads made of the objects it
> hands over, and how the run terminated. Any component of that trace which varies with the
> branch is a channel of exactly the width of its variation. So: over the positions of one
> link, the trace must take exactly **one** value. A constant carries no bits.
...
It found route I on its first run, which is the evidence that it's more than a restatement.
