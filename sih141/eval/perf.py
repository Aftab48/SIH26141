...
Two hundred trials at production parameters, single-threaded and at twenty
workers on this laptop's realistic 15x effective speedup:
...
quoted without both is not a figure. An unchecked full-scale run is 0.226 KB per
...
"""Mapping: Transcript JSON size in KB per position, by ``(L, check_fraction)``.

Keyed by both, because **the check fraction is half the answer** and quoting the
figure without it is how a correct number becomes a wrong one. Check rounds
publish a channel sample per round -- fidelity, purity, concurrence, both wing
purities -- so a quarter-checked run carries about 46% more transcript per
position than an unchecked one at the same length.

At ``check_fraction = 0.25`` the size falls with ``L`` towards 0.33 KB per
position as the fixed header is amortised. At ``check_fraction = 0`` -- which is
what :data:`~sih141.protocol.params.DEFAULT_PARAMS` uses -- it is 0.226.

This entry has been wrong once already, in this module, in the other direction.
A first pass recorded 0.226 as *the* full-scale figure and wrote that the Phase 5
brief's 0.33 was an error. The brief was right: it had measured a checked run.
Both numbers were correct measurements of two different configurations, and the
mistake was reporting one of them without its second coordinate.
"""
...
1.54 s, dominated by reading 26.7 MB of JSON, against 2.9 ms on a 96-position
...
def peak_rss_bytes() -> int | None:
    ...
    of these fit in 24 GB", and the answer is about the high-water mark.
...
better. Treat 7.04x as measured here and as a plausible ceiling there.
"""
...
its transcript is 26.7 MB rather than 37.2, so the full-scale peak there is
lower than the last row.

The floor is about 76 MB -- numpy and qiskit imported, before any session -- and
the rise above it is roughly linear in ``L``. At full scale one worker peaks at
349 MB, so twenty are about 7 GB against this machine's 24. That fits, but it is
not the "under 2 GB" the phase brief records: that figure is twenty import
